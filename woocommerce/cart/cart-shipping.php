	...
	<td>
		<?php if ( ! empty( $available_methods ) ) : ?>
			...
			<?php else : ?>
				...
				<ul id="shipping_method"  class="list-unstyled">
					<?php foreach ( $available_methods as $method ) : ?>
						<li class="radio">
							<label>
							<input type="radio" name="shipping_method[<?php echo $index; ?>]" data-index="<?php echo $index; ?>" id="shipping_method_<?php echo $index; ?>_<?php echo sanitize_title( $method->id ); ?>" value="<?php echo esc_attr( $method->id ); ?>" <?php checked( $method->id, $chosen_method ); ?> class="shipping_method" /> <?php echo wp_kses_post( wc_cart_totals_shipping_method_label( $method ) ); ?>
							</label>
						</li>
					<?php endforeach; ?>
				</ul>
